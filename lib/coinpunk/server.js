...
server.get('/api/tx/unspent', function(req,res) {
  if(req.query.confirmations)
    var confirmations = parseInt(req.query.confirmations);
  else
    var confirmations = 1;
  
  bitcoind.rpc('listunspent', [confirmations, 99999999999999, req.query.addresses], function(err, btcres) {
    ...
    for(var i=0;i<btcres.length; i++) {
      unspent.push({
        ...
        amountSatoshiString: new BigNumber(btcres[i].amount).times(Math.pow(10,8)).toString()
      });

      amount = amount.plus(btcres[i].amount);
    }

    res.send({unspent: unspent, amount: amount.toString()});
  });
});
