...
server.get('/api/dashboard', function(req,res) {
  ...
  var receiveAddresses = req.query.receiveAddresses;
  
  bitcoind.batch([
    {method: 'getaddressesbyaccount', params: [req.query.serverKey]},
    {method: 'listtransactions', params: []},
    {method: 'listunspent', params: [0, 9999999, addresses]},
    ...
    for(i=0;i<results[1].result.length;i++) {
      for(var v=0;v<receiveAddresses.length;v++) {
        if(results[1].result[i].address == receiveAddresses[v])
