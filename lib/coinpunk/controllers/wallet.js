...
var WalletController = function (root, express, db, bitcoind, websocket) {
  var that = this;
  ...
  controller.Controller.call(that, root, express, db);
};
...
WalletController.prototype.saveWalletAndAddresses = function(req, res) {
  ...
  if(req.body.address)
    return that.bitcoind.rpc('importaddress', [req.body.address, req.body.serverKey, false], function(err, btcres) {
      if(err)
