// A means to iteratively adjust an image of a qr to ensure it can be scanned.
//
// Author: Ian Coleman - thedawnrider on github
// Date: 2014-02-17
// Dependencies - jsqrcode - https://github.com/LazarSoft/jsqrcode
//
// Usage:
// new QrImageDecoder({
...
//     }
// })
function QrImageDecoder(options) {

    // Private Properties

    var This = this;
    var maxCanvasSize = 256;
    var initialBlur = 0;
    var currentBlur = 0;
    var blurStep = 0.075;
    var maxBlur = 0.14; // 0-5
    ...
    function enhance() {
        This.debug.iterations += 1;
        currentBlur += blurStep;
        Pixastic.process(originalImage, "blurfast", {
            amount: currentBlur
        }, function(img) {
            drawAndDecode(img)
        });
    ...
    function init() {
        ...
        if (!("src" in options)) {
            throw("QrImageDecoder requires 'src' in options");
        }

        // Global variable from jsqrcode - not a happy camper.
        qrcode.callback = function(result) {
            var didDecode = result != "error decoding QR Code";
            if (didDecode) {
                reset();
        ...
        originalImage.onload = function() {
            // Calculate values
            var isLarge = originalImage.width > maxCanvasSize
            canvasWidth = isLarge ? maxCanvasSize : originalImage.width;
            var imgRatio = originalImage.height / originalImage.width;
            canvasHeight = canvasWidth * imgRatio;
            canvas.width = canvasWidth;
            canvas.height = canvasHeight;
            // Resize image
            drawAndDecode(originalImage);
        }
